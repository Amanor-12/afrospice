...
const DEFAULT_MONGOD_PATH = "C:\\Program Files\\MongoDB\\Server\\8.2\\bin\\mongod.exe";
const LOCAL_MONGOD_PATH = process.env.LOCAL_REPLICA_MONGOD_PATH || DEFAULT_MONGOD_PATH;
...
function startLocalReplicaProcess() {
  ...
  if (!fs.existsSync(LOCAL_MONGOD_PATH)) {
    throw new Error(
      `mongod binary was not found at ${LOCAL_MONGOD_PATH}. Set LOCAL_REPLICA_MONGOD_PATH if needed.`
    );
  }
...
async function ensureLocalReplicaSet(uri = process.env.MONGO_URI || runtime.mongoUri) {
  ...
  if (!online) {
    throw new Error("Local replica-set mongod did not become reachable on port 27018.");
  }
