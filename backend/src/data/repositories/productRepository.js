...
function normalizeProduct(row) {
  ...
  return {
    ...
    receivingNotes: String(row.receivingNotes || "").trim(),
...
async function createProduct(product) {
  ...
  await models.Product.create({
    ...
    unitLabel: String(product.unitLabel || "").trim(),
    casePack: Number(product.casePack || 0),
    reorderPoint: Number(product.reorderPoint || 0),
    parLevel: Number(product.parLevel || 0),
    shelfLocation: String(product.shelfLocation || "").trim(),
    receivingNotes: String(product.receivingNotes || "").trim(),
